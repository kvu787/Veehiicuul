...
try {
    ...
    Invoke-Checked 'dotnet' @('build', 'InputLatencyGodot.slnx', '--configuration', 'ExportRelease', '--nologo', '-warnaserror')
    ...
    Write-Host "Built application: $(Join-Path $PSScriptRoot 'Build\InputLatencyGodot.exe')"
}
catch {
